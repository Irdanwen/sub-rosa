...
- [adr/0053-a-share-is-a-dated-envelope-the-server-cannot-open.md](adr/0053-a-share-is-a-dated-envelope-the-server-cannot-open.md) — showing somebody a note: a key per share in the URL fragment, ciphertext on the account service, a mandatory deadline, two public reads that answer by position and never by blob id, and the release job that hands the bytes back to the quota
- [adr/0054-an-errand-runs-on-the-device-that-has-the-means.md](adr/0054-an-errand-runs-on-the-device-that-has-the-means.md) — the phone hands a link to the device that has the extractor: a tenth kind, addressed to one device, accepted only where the owner switched errands on, single use through a local unsynchronised ledger, and the one deliberate exception to ADR-0049's inert incoming state
- [adr/0055-a-native-sign-in-comes-back-through-a-deep-link.md](adr/0055-a-native-sign-in-comes-back-through-a-deep-link.md) — signing in from the app: the real page opens and hands the session back through `subrosa://`, with the PKCE verifier and a return code as two halves that only work together; supersedes the universal-link reason ADR-0049 gave for the eight character code
...
- [adr/0059](adr/0059-film-projects-are-local-versioned-documents.md) — local film project documents, atomic save revisions, project-owned bible copies and durable gallery metadata
