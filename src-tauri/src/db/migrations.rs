...
pub async fn run_migrations(_pool: &SqlitePool) -> Result<(), sqlx::error::Error> {
    ...
    replay(
        _pool,
        "025_errands.sql",
        include_str!("../../migrations/025_errands.sql"),
    )
    .await?;
    for table in [
        "account_sync_outbox",
    ...
    replay(
        _pool,
        "031_agent_task_titles.sql",
        include_str!("../../migrations/031_agent_task_titles.sql"),
    )
    .await?;
    replay(
        _pool,
        "032_studio_projects.sql",
