...
// the audited pin-to-tag compatibility delta. (The fork bundles the pinned
// runtime unpatched, so upstream's HERMES_RUNTIME_PATCH_SET / patched-source
...
pub(crate) const JUNE_SOUL_BLOCKS_MD: &str = r#"
...
Follow-up cards: after a meeting note, or when the user agrees to something, you may end your reply with one fenced block whose info string is `subrosa:proposal` and whose body is {"v":1,"proposalId":"<a new short id>","title":"Follow-ups","actions":[{"kind":"reminder","id":"a1","label":"…","due":"<RFC3339>"},{"kind":"event","id":"a2","label":"…","start":"<RFC3339>"},{"kind":"note","id":"a3","label":"…","noteId":"<a real note id>","text":"…"},{"kind":"summarize","id":"a4","label":"…","noteId":"<a real note id>"},{"kind":"importLink","id":"a5","label":"…","url":"https://…"}]}. Nothing happens until the user clicks a card, so propose rather than announce: never write as if the reminder already exists. Five actions at most, only ones the conversation actually calls for, and never invent a note id. Two of the kinds spend the user's money and take minutes: `summarize` reads one of their long recordings end to end and is for a talk, a lecture or a podcast rather than a meeting, and `importLink` fetches a podcast feed, a podcast episode or a direct audio or video URL (streaming platform pages such as YouTube or Spotify do not work). Propose those two rather than describing their result, and never propose `summarize` for something `read_note` already answers.
...
/// Appended to `SOUL.md` for every runtime: how a film gets made in the app.
///
/// The remote studio note this replaces taught the agent to negotiate a DIEM
/// budget with a service that is being retired. This one describes the local
/// path, and its shape is deliberate: the detail lives in the
/// `subrosa-production` skill, which costs nothing until it is read, so this
/// note only has to be enough for the agent to know the skill exists and when
/// to reach for it. Same no-"sandbox"-wording constraint as the media note.
...
fn hermes_runtime_ownership_dir(app: &AppHandle) -> Result<PathBuf, AppError> {
    app.path().app_config_dir().map_err(|error| {
        AppError::new(
            ...
            format!("Could not resolve the Hermes runtime ownership directory: {error}"),
        )
    })
}
...
fn probe_hermes_start_time(pid: u32) -> HermesStartTimeProbe {
    ...
    match output {
        ...
        Ok(output) if !output.status.success() => HermesStartTimeProbe::Gone,
        Ok(output) => {
...
fn probe_hermes_process_group(pid: u32) -> HermesProcessGroupProbe {
    ...
    match output {
        ...
        Ok(output) if !output.status.success() => HermesProcessGroupProbe::Gone,
        Ok(output) => {
...
fn record_spawned_hermes_runtime_in_dir(
    ...
    let result = (|| {
        ...
        let HermesStartTimeProbe::StartedAt(runtime_start) = probe_hermes_start_time(runtime_pid)
            ...
            return Err(AppError::new(
                "hermes_runtime_owner_unrecordable",
                "Could not read the Hermes runtime process start time.",
            ));
        };
        if !matches!(
            ...
            return Err(AppError::new(
                "hermes_runtime_owner_unrecordable",
                "The Hermes runtime was not its process-group leader.",
            ));
        }
...
fn reap_orphaned_hermes_runtimes_in_dir(directory: &Path) -> Result<(), AppError> {
    let entries = match fs::read_dir(directory) {
        ...
        Err(error) => {
            return Err(AppError::new(
                ...
                format!("Could not read Hermes runtime ownership records: {error}"),
            ));
        }
    };
    for entry in entries {
        let entry = entry.map_err(|error| {
            AppError::new(
                "hermes_runtime_reap_failed",
                format!("Could not enumerate Hermes runtime ownership records: {error}"),
            )
        })?;
        let path = entry.path();
        let is_record = path
        ...
        let raw = fs::read(&path).map_err(|error| {
            AppError::new(
                "hermes_runtime_reap_failed",
                format!(
                    "Could not read Hermes runtime ownership record {}: {error}",
                    path.display()
                ),
            )
        })?;
        let record: HermesRuntimeOwnershipRecord =
            serde_json::from_slice(&raw).map_err(|error| {
                AppError::new(
                    "hermes_runtime_reap_failed",
                    format!(
                        "Malformed Hermes runtime ownership record {}: {error}",
                        path.display()
                    ),
                )
            })?;
        if path.file_name() != hermes_runtime_ownership_path(directory, &record).file_name() {
            return Err(AppError::new(
                "hermes_runtime_reap_failed",
                format!(
                    "Hermes runtime ownership record identity did not match its filename: {}",
                    path.display()
                ),
            ));
        }
        ...
        match action {
            ...
            HermesOwnershipAction::Reap => {
                ...
                if !wait_for_recorded_hermes_runtime_exit(
                    ...
                    return Err(AppError::new(
                        ...
                        "An orphaned Hermes runtime process group would not exit.",
                    ));
                }
                remove_hermes_ownership_record(Some(&path)).map_err(|error| {
                    AppError::new("hermes_runtime_reap_failed", error.to_string())
            ...
            HermesOwnershipAction::Abort => {
                return Err(AppError::new(
                    "hermes_runtime_reap_failed",
                    "Could not establish Hermes runtime process ownership.",
                ));
            }
        }
    }
    ...
    loop {
        match probe_hermes_start_time(pid) {
            HermesStartTimeProbe::Unknown => {
                return Err(AppError::new(
                    "hermes_runtime_reap_failed",
                    "Could not confirm the orphaned Hermes runtime exited.",
                ));
            }
...
fn sync_june_studio_mcp(
    ...
    hermes_command: &str,
    coordinates_path: &Path,
) -> Result<JuneStudioMcpConfig, AppError> {
    let data_dir = crate::app_paths::app_data_dir(app)
...
///
/// The MCP names an action and passes JSON. It never sees a key, a path or a
/// gallery file: everything it can reach is a command the app already exposes
/// to its own webview.
async fn forward_studio_request(
    app: &AppHandle,
    stream: &mut tokio::net::TcpStream,
    request_body: &[u8],
) -> io::Result<()> {
    let payload = match serde_json::from_slice::<serde_json::Value>(request_body) {
        ...
        Err(error) => {
            return write_json_response(
                ...
                400,
                serde_json::json!({
                    "error": { "message": format!("Invalid studio request: {error}") }
                }),
            )
            .await;
        }
    ...
    match crate::studio_actions::dispatch(app.clone(), &action, params).await {
        Ok(value) => write_json_response(stream, 200, value).await,
        Err(error) => {
            let status = match error.code.as_str() {
                "studio_unknown_action" | "studio_invalid_params" | "bible_invalid" => 400,
                _ => 502,
            };
            write_json_response(
                stream,
...
mod tests {
    ...
    fn dead_hermes_owner_with_gone_runtime_deletes_stale_record() {
        let directory = tempfile::tempdir().expect("tempdir");
        let record = hermes_test_record(4_000_000, "dead-owner", 4_000_001, "gone", false);
        let path = write_hermes_runtime_ownership_record(directory.path(), &record)
            .expect("write ownership record");
    ...
    #[test]
    fn the_agent_is_told_where_film_production_went_and_what_it_cannot_do() {
        // The swap has to be atomic with the tool: an agent told to call a
        // tool that refuses retries instead of explaining, and an agent given
        // a tool nobody told it about never reaches for it.
        assert!(JUNE_SOUL_STUDIO_MD.contains("june_studio"));
        assert!(JUNE_SOUL_STUDIO_MD.contains("subrosa-production"));
        assert!(JUNE_STUDIO_MCP_SCRIPT.contains("\"name\": \"bible\""));
        assert!(JUNE_STUDIO_MCP_SCRIPT.contains("\"name\": \"shots\""));
        // It must not promise to spend: compiling and confirming are the
        // user's, in the Studio.
        assert!(JUNE_SOUL_STUDIO_MD.contains("You cannot start a render"));
    }

    #[test]
    fn the_studio_toolset_stays_small_enough_to_carry_on_every_turn() {
        // The whole surface is in context on every turn of every conversation,
        // so its size is a running cost. Action-discriminated is what keeps it
        // to a few hundred tokens instead of a few thousand; this is the
        // ratchet that stops it growing back into thirty granular tools.
        let tool_names = JUNE_STUDIO_MCP_SCRIPT.matches("\"name\": \"").count();
        assert!(
            tool_names <= 4,
            "the studio toolset grew to {tool_names} tools"
        );
    }

    fn test_june_studio_mcp_config() -> JuneStudioMcpConfig {
        JuneStudioMcpConfig {
            command: "/tmp/hermes/venv/bin/python".to_string(),
            script_path: PathBuf::from("/tmp/june/hermes-mcp/june_studio_mcp.py"),
            coordinates_path: PathBuf::from("/tmp/june/hermes-mcp/june_web_proxy.json"),
        }
    }
        ...
        for root in [
            ...
            for memory_enabled in [true, false] {
                ...
                let coordinates_path =
                    PathBuf::from(format!(r"{root}\hermes-mcp\june_web_proxy.json"));
