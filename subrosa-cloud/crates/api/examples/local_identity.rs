...
async fn token(
    ...
    let key = EncodingKey::from_rsa_pem(include_bytes!("../tests/fixtures/oidc-test-only.pem"))
        .map_err(|_| axum::http::StatusCode::INTERNAL_SERVER_ERROR)?;
    let mut header = Header::new(Algorithm::RS256);
    header.kid = Some("test-key".into());
    let now = Utc::now().timestamp();
    let token=encode(&header,&json!({"iss":"http://127.0.0.1:8788","sub":"local-qa-user","aud":"test-client","email":"qa@example.test","email_verified":true,"nonce":auth.nonce,"iat":now,"exp":now+300,"auth_time":now}),&key).map_err(|_|axum::http::StatusCode::INTERNAL_SERVER_ERROR)?;
...
async fn main() -> Result<()> {
    ...
    let oidc=Router::new().route("/.well-known/openid-configuration",get(||async{Json(json!({"issuer":"http://127.0.0.1:8788","authorization_endpoint":"http://127.0.0.1:8788/authorize","token_endpoint":"http://127.0.0.1:8788/token","jwks_uri":"http://127.0.0.1:8788/jwks"}))})).route("/jwks",get(||async{( [(axum::http::header::CONTENT_TYPE,"application/json")],include_str!("../tests/fixtures/jwks.json"))})).route("/authorize",get(authorize)).route("/approve",post(approve)).route("/token",post(token)).with_state(issuer);
