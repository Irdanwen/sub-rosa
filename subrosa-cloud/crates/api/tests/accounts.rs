...
impl Fixture {
    ...
    async fn login(&self, subject: &str) -> Result<Browser> {
        ...
        let key = EncodingKey::from_rsa_pem(include_bytes!("fixtures/oidc-test-only.pem"))?;
        // OpenSSL test key's public modulus is generated by the fixture helper once and checked in.
        let jwks: Value = serde_json::from_str(include_str!("fixtures/jwks.json"))?;
...
async fn oidc_rejects_substituted_browser_nonce_audience_and_signature() -> Result<()> {
    ...
    for mutation in ["nonce", "aud", "email_verified", "signature", "auth_time"] {
        ...
        let mut header = Header::new(Algorithm::RS256);
        header.kid = Some("test-key".into());
        let mut token = encode(&header, &claims, &key)?;
        ...
        Mock::given(method("GET"))
            .and(path("/jwks"))
            .respond_with(
                ResponseTemplate::new(200).set_body_json(serde_json::from_str::<Value>(
                    include_str!("fixtures/jwks.json"),
                )?),
            )
            .mount(&f.idp)
            .await;
