...
| `src-tauri/windows/installer-hooks.nsh` (nouveau) + `src-tauri/tauri.windows.conf.json` (NSIS kill-runtime, 2026-07-08) | **Fix « Error opening file for writing … hermes.exe » à l'install Windows** : sur une mise à jour/réinstallation, l'installeur NSIS ne peut pas écraser `$INSTDIR\native\hermes\bin\hermes.exe` (ni `python\current\python.exe`) tant que le runtime Hermes tourne — Windows verrouille tout exécutable actif. Le template NSIS de Tauri ne ferme que le process principal (`Sub Rosa.exe`) et ignore les enfants `hermes.exe`/`python.exe` que l'app a spawnés (et qui survivent à un crash/force-quit). Idem pour les `.pyd` du venv (extensions natives = DLL mappées par un `python.exe` vivant : `…\hermes-agent\venv\Lib\site-packages\aiohttp\_http_parser.cp311-win_amd64.pyd`). Hook `NSIS_HOOK_PREINSTALL` (+ `PREUNINSTALL`) : `Stop-Process` de **tout process dont le chemin est sous `$INSTDIR\native\hermes`**, quel que soit son nom (`.Path.StartsWith('$INSTDIR\native\hermes')` → couvre `hermes.exe`/`python.exe`/`pythonw.exe`/`python3.11.exe` ; un Python utilisateur ailleurs n'est jamais touché) + `Sleep` pour laisser Windows libérer les handles. Wiré via `nsis.installerHooks`. | Réappliquer le `.nsh` + la clé `installerHooks` ; garder le filtre par chemin (sous `native\hermes`, pas par nom de process) |
