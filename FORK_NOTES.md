...
| `june-api/Cargo.toml` + `crates/{domain,providers,services,api}/Cargo.toml` (dépendances `bytes`, `futures-core`, `futures-util`, déjà dans le lock), `june-api/crates/domain/src/lib.rs`, `june-api/crates/providers/src/{venice,lib}.rs`, `june-api/crates/services/src/{agent_chat,charge_flow}.rs`, `june-api/crates/api/src/handlers/agent.rs`, `june-api/crates/api/tests/http_boundary.rs`, `june-api/crates/embed/src/lib.rs`, `src-tauri/src/{june_api,lib,hermes_bridge,agent_lite/mod,ask/mod,note_ai/mod}.rs`, `src/components/{mobile/screens/AgentScreen,assistants/AssistantChat}.tsx`, `src-tauri/Cargo.lock` (**le sidecar streame le chat agent**, 2026-09-25, [ADR-0063](docs/adr/0063-the-sidecar-streams-agent-chat.md)) | **Le premier token n'arrivait qu'à la fin de la génération** : `complete_raw_once` lisait tout le corps (`response.bytes()`), le service facturait, puis le handler répondait — le shell streamait déjà vers Hermes, mais n'avait rien à streamer. Sur un tour d'agent (outil → nouvelle complétion → …) l'attente = durée totale × nombre d'étapes. Désormais, **si le client demande `stream:true` ET que l'amont répond 2xx `text/event-stream`**, le provider relaie le flux tel quel (`agent_chat_stream.rs`, ajouté) avec un scanner ligne à ligne (découpe sur l'octet `\n`, frame coupée entre chunks tolérée, une seule ligne partielle ≤ 256 KiB, seules les lignes contenant `"usage"` sont parsées, dernier `usage` non nul retenu via `token_usage_from_value` passé `pub(crate)` ; `usage_from_sse` réutilise ce scanner). Domaine : `AgentChatCompleter::complete` rend `AgentChatResponse` = `Buffered(AgentChatCompletion)` (struct inchangée, `.into()`) \| `Streamed(AgentChatStream { body, content_type, provider, usage: future })`. Service : autorisation AVANT l'appel (inchangé) ; un flux est rendu tout de suite et une tâche attend l'usage puis `price_settled_work` + `clamp_to_cap` + `settle_charge` (passé `pub(crate)`), même clé d'idempotence ; **flux abandonné = facturé sur l'usage vu (souvent 0)**, choix documenté. Handler : `Body::from_stream`, content-type amont, **pas d'en-têtes `x-june-*`** sur ce chemin (le chemin bufferisé les garde). Retries/backoff/rejeu sans `stream_options` inchangés (décidés sur le statut, avant tout octet) ; erreur de lecture en cours de flux = rapport d'usage puis **item d'erreur** (le corps passe en `Result<Bytes, io::Error>`, hyper coupe le chunked sans terminateur), jamais de rejeu côté sidecar ; `TCP_NODELAY` via `tap_io` dans `june-embed::serve_config` ; pas de warn « no usage frame » sur un flux abandonné (Arrêter). `/router` (JSON → synthèse SSE) et clients non-stream inchangés. Shell : `carpe_diem/stream_usage.rs` (ajouté) lit l'usage dans la dernière frame quand les en-têtes manquent (en-têtes prioritaires), `sse_lines.rs` (ajouté) découpe les SSE sur les octets pour agent-lite, ask et note_ai (un « é » coupé entre deux chunks devenait U+FFFD), agent-lite ignore un `index` de tool_call ≥ 64, et le proxy sérialise le corps une seule fois. **Un flux n'est une réponse que s'il dit qu'il a fini** (`finish_reason` ou `[DONE]`, `sse_lines::CompletionFrames`/`read_content`) : sinon `reply_cut_off`, jamais une réponse courte ni un tool call aux arguments tronqués ; agent-lite rejoue une fois l'itération dont le corps casse (verrouillage d'écran) après avoir envoyé au webview un `retract` (unités UTF-16, `src/lib/agent-lite-delta.ts`) ; le proxy Hermes termine un flux cassé par une frame `data: {"error":…}` (`stream_relay.rs`). | Réappliquer : le type `AgentChatResponse` + ses `From` (domain), la branche « client stream + amont SSE » en tête de la lecture du corps dans `complete_raw_once` (juste après le test de statut, AVANT `response.bytes()`) + `.into()` sur le chemin bufferisé, `usage_from_sse` via le scanner, le `match` du service et l'enum `AgentChatOutput`, le `match` du handler ; tout mock `AgentChatCompleter` venu d'upstream doit rendre `.into()`. Si upstream touche à `complete_raw_once`, garder la branche de relais avant la lecture du corps |
...
| `june-api/crates/providers/src/agent_chat_stream.rs`, `src-tauri/src/carpe_diem/stream_usage.rs`, `src-tauri/src/sse_lines.rs`, `src-tauri/src/stream_relay.rs`, `src/lib/agent-lite-delta.ts`, `docs/adr/0063-the-sidecar-streams-agent-chat.md` (**chat agent streamé**, 2026-09-25) | Relais SSE du sidecar avec scanner d'usage borné (côté june-api, fichier ajouté pour ne pas grossir `venice.rs`) ; lecture de l'usage dans la dernière frame côté shell quand les en-têtes `x-june-*` manquent ; découpe SSE sur les octets partagée par agent-lite, ask et note_ai. Voir l'entrée ADR-0063 des fichiers upstream modifiés. |
...
the draft v1.63.0 Intel DMG exposed this during artifact inspection.
The release workflow now rejects a mismatched host and checks every bundled
Mach-O runtime file with `lipo -verify_arch` before signing. Keep these gates
