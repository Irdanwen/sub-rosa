...
| `src/components/studio/FilmDirectorPanel.tsx` | Panneau réalisateur par projet : rail des phase gates (approve/reject motivé), chat avec l'équipe du studio (progrès outil en live, prop `initialDraft` semée par FilmStudio ; « Attach image » = staging rôle+nom avant upload → `filmRefLine` dans le brouillon ; « Improve with AI » sur le message avec aperçu accepter/rejeter — `mode:"brief"` sur le 1er message d'un projet vierge, `mode:"direction"` ensuite), handshake produce (quote → confirmation), board plans/scènes (vignettes, prises, retake/retry/skip) |
