...
| `src-tauri/src/lib.rs`, `src-tauri/Cargo.toml`, `src-tauri/tauri.conf.json` (CSP), `src-tauri/src/carpe_diem/{settings,media}.rs`, `src/components/settings/AppSettings.tsx`, `src/components/sidebar/Sidebar.tsx`, `src/components/studio/StudioView.tsx`, `src/lib/studio/artifacts.ts`, `src/lib/tauri.ts`, `src/styles/studio.css` (**Films / Videomaker**, 2026-07-11/12) | Production de films via Videomaker Studio (voir `docs/adr/0010-videomaker-film-production.md`) : `#[cfg(desktop)] pub mod videomaker;` + `videomaker::setup(app)` + 32 commandes dans la **seule** liste desktop de `generate_handler!` ; deps `k256`/`sha3` (signature SIWE) ; CSP : `img-src`/`media-src` élargis à `https:` (vignettes storyboard + clips signés du board — `script-src` reste verrouillé par hash) ; hooks `on_carpe_diem_key_changed`/`_cleared` dans `carpe_diem::settings` (re-push/suppression de la clé côté Videomaker) ; `artifacts_dir` passé `pub(crate)` dans `carpe_diem::media` (le watcher y télécharge les films finis) ; onglet Settings « Film studio » (AppSettings + `SETTINGS_SIDEBAR_GROUPS`) ; onglet « Films » dans StudioView (rendu hors gate catalogue) ; `registerDownloadedArtifact` dans `lib/studio/artifacts.ts` (indexer un fichier déjà écrit par Rust) ; wrappers IPC `videomaker*` dans `tauri.ts` ; styles `.film-*` | Réappliquer : mod+setup+32 commandes (desktop uniquement), le patch CSP, 2 hooks settings, `pub(crate)`, 2 entrées UI, l'onglet Films, la fn artifacts, les wrappers, le bloc CSS |
...
| `src/components/studio/FilmStudio.tsx` | Surface Films du Studio : brief → production autonome (plafond DIEM obligatoire) ou mode dirigé (switch), images de référence (≤ 4, rôle personnage/lieu/style/objet + libellé, uploadées après création puis manifeste ajouté au brief du run ou semé dans le chat réalisateur ; échec d'upload = run non lancé, jamais de référence perdue en silence), « Improve with AI » (aperçu accepter/rejeter, jamais d'écrasement du brouillon), suivi live (événements du watcher), téléchargement dans la galerie, suppression |
