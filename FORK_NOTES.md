...
| `src-tauri/tauri.conf.json` (fenêtre `main`, **Fix WebSocket Hermes sur PC managé Windows**, 2026-07-08) | **Local Network Access d'Edge bloque le chat** : sur un PC d'entreprise, la stratégie Edge/WebView2 `LocalNetworkAccessAllowedForUrls` (fonctionnalité « Local Network Access » / ex-Private Network Access) interdit à toute origine hors liste blanche d'atteindre le réseau local. Le chat s'affiche dans WebView2 (moteur Edge), donc son `ws://127.0.0.1:<port>/api/ws` vers le dashboard Hermes est **bloqué** → bannière « Could not connect to Hermes gateway ». Le modèle (Carpe Diem) marche quand même car il passe par le code natif Rust (`reqwest` + `.no_proxy()`), non soumis à WebView2. Diagnostiqué via `edge://policy` chez un utilisateur (`LocalNetworkAccessAllowedForUrls` = leur seul SharePoint). **Fix** : `additionalBrowserArgs` sur la fenêtre `main` désactive la fonctionnalité pour le webview de l'app (`--disable-features=…,LocalNetworkAccessChecks,PrivateNetworkAccessSendPreflights,PrivateNetworkAccessRespectPreflightResults,BlockInsecurePrivateNetworkRequests`), en **conservant** les défauts Tauri (`msWebOOUI,msPdfOOUI,msSmartScreenProtection`) qu'un `additionalBrowserArgs` explicite écraserait sinon. No-op hors Windows. Contournement immédiat sans rebuild : l'IT ajoute l'origine de l'app (`http://tauri.localhost`) à la stratégie, ou variable d'env `WEBVIEW2_ADDITIONAL_BROWSER_ARGUMENTS`. | Garder l'arg ; si Tauri change ses défauts, re-fusionner la liste `msWebOOUI,…` |
