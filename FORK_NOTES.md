...
| `src-tauri/src/lib.rs`, `src-tauri/Cargo.toml`, `src-tauri/tauri.conf.json` (CSP), `src-tauri/src/carpe_diem/{settings,media}.rs`, `src/components/settings/AppSettings.tsx`, `src/components/sidebar/Sidebar.tsx`, `src/components/studio/StudioView.tsx`, `src/lib/studio/artifacts.ts`, `src/lib/tauri.ts`, `src/styles/studio.css` (**Films / Videomaker**, 2026-07-11/12) | Production de films via Videomaker Studio (voir `docs/adr/0010-videomaker-film-production.md`) : `#[cfg(desktop)] pub mod videomaker;` + `videomaker::setup(app)` + 30 commandes dans la **seule** liste desktop de `generate_handler!` ; deps `k256`/`sha3` (signature SIWE) ; CSP : `img-src`/`media-src` élargis à `https:` (vignettes storyboard + clips signés du board — `script-src` reste verrouillé par hash) ; hooks `on_carpe_diem_key_changed`/`_cleared` dans `carpe_diem::settings` (re-push/suppression de la clé côté Videomaker) ; `artifacts_dir` passé `pub(crate)` dans `carpe_diem::media` (le watcher y télécharge les films finis) ; onglet Settings « Film studio » (AppSettings + `SETTINGS_SIDEBAR_GROUPS`) ; onglet « Films » dans StudioView (rendu hors gate catalogue) ; `registerDownloadedArtifact` dans `lib/studio/artifacts.ts` (indexer un fichier déjà écrit par Rust) ; wrappers IPC `videomaker*` dans `tauri.ts` ; styles `.film-*` | Réappliquer : mod+setup+30 commandes (desktop uniquement), le patch CSP, 2 hooks settings, `pub(crate)`, 2 entrées UI, l'onglet Films, la fn artifacts, les wrappers, le bloc CSS |
...
| `src-tauri/src/videomaker/{mod,wallet,auth,client,commands,events,projects,director}.rs` | **Films (ADR-0010)** : wallet Ethereum app-managed (secp256k1 keychain, signature EIP-191, checksum EIP-55, message SIWE), chaîne d'activation (session cookie SIWE → `POST /me/key` → PAT `vmk_`), client typé (idempotency keys, self-heal 401, taxonomie d'erreurs stable dont 409 quote / 423 final review), réglages `videomaker.json` (slugs suivis, films exportés), commandes settings + compte, commandes projets/runs/produce/export, watcher SSE par projet (backoff, resync `/status`, auto-download du final cut dans la galerie), mode réalisateur (`director.rs` : gates approve/reject, board, takes/retake/requeue/skip, chat streamé `POST /chat/stream` re-émis en `june://videomaker-chat`) |
| `src/lib/films/index.ts` | Types + parseurs défensifs des payloads Videomaker (projets, status, runs, handshake produce, gates, transcript, board, takes) + abonnements `june://videomaker-event` / `june://videomaker-chat` |
...
| `src/components/studio/FilmStudio.tsx` | Surface Films du Studio : brief → production autonome (plafond DIEM obligatoire) ou mode dirigé (switch), suivi live (événements du watcher), téléchargement dans la galerie, suppression |
| `src/components/studio/FilmDirectorPanel.tsx` | Panneau réalisateur par projet : rail des phase gates (approve/reject motivé), chat avec l'équipe du studio (progrès outil en live), handshake produce (quote → confirmation), board plans/scènes (vignettes, prises, retake/retry/skip) |
| `src/test/videomaker-settings.test.tsx`, `src/test/film-studio.test.tsx`, `src/test/film-director.test.tsx` | Tests UI réglages + parcours autonome + mode réalisateur + parseurs |
