...
export async function notifyRecordingStillMeetingPrompt(sessionId: string) {
  ...
  try {
    await sendNotification({
      ...
      body: `Sub Rosa will pause the recording in ${RESPONSE_SECONDS} seconds if you do not answer.`,
...
export async function notifyRecordingAutoPaused(sessionId: string) {
  ...
  try {
    await sendNotification({
      title: "Sub Rosa paused recording",
      body: "No meeting audio was detected. Open Sub Rosa to resume or finish.",
      group: `june-recording-${sessionId}`,
      sound: NOTIFICATION_SOUND,
    });
    return true;
