...
import { MediaError, mediaGet, mediaJson, mediaRaw } from "../../lib/studio/client";
...
function isHeavyImageModel(modelId: string): boolean {
...
export function ImageStudio({ catalog }: { catalog: MediaCatalog }) {
  ...
  const generate = useCallback(async () => {
    ...
    try {
      let images: string[];
      if (isHeavyImageModel(model.id)) {
        images = await generateViaQueue(body);
      } else {
        try {
          images = imagesFromResponse(await mediaJson<GenerateResponse>("/image/generate", body));
        } catch (syncError) {
          // A 502 on a sync call is usually the edge cap, not a failure:
          // the async path renders the same request without the cap.
