...
export function AgentWorkspace({
  ...
  const [homeDirectTurns, setHomeDirectTurns] = useState<AgentChatTurn[]>(() =>
    homeMode ? readHomeDirectTurns(initialSessionIdProp) : [],
  );
  const homeDirectTurnsRef = useRef(homeDirectTurns);
  const [homeTaskHandoffs, setHomeTaskHandoffs] = useState<HomeTaskHandoff[]>(() =>
    homeMode ? readHomeTaskHandoffs(initialSessionIdProp) : [],
  );
  const homeTaskHandoffsRef = useRef(homeTaskHandoffs);
  ...
  useEffect(() => {
    ...
    const restoredTurns = readHomeDirectTurns(selectedHermesSessionId);
    const restoredHandoffs = readHomeTaskHandoffs(selectedHermesSessionId);
    homeDirectTurnsRef.current = restoredTurns;
    homeTaskHandoffsRef.current = restoredHandoffs;
  ...
  const {
    ...
    localGeneration,
    newSessionMode,
    onSessionSelected: homeMode ? undefined : onSessionSelected,
    pendingHermesMessages,
    selectedHermesSessionId,
    selectedTaskId,
    sessionModelSelections,
  ...
  useLayoutEffect(() => {
    ...
    const contentObserver =
      typeof MutationObserver === "function" ? new MutationObserver(scheduleMeasure) : undefined;
  ...
  }, [activePanel, homeMode]);

  useEffect(() => {
    if (!import.meta.env.DEV || !homeMode) return;
    function refreshHomeDemo() {
      const profile = activeHermesProfile.name;
      const storedSessionId =
        readJuneHomeStoredSessionId(profile) ?? selectedHermesSessionIdRef.current;
    ...
    window.addEventListener(HOME_DEMO_SEEDED_EVENT, refreshHomeDemo);
    return () => window.removeEventListener(HOME_DEMO_SEEDED_EVENT, refreshHomeDemo);
  }, [activeHermesProfile.name, homeMode, onHomeSessionCreated]);
  ...
  async function ensureHomeSession(): Promise<string> {
    const selected = selectedHermesSessionIdRef.current;
  ...
  function commitHomeDirectTurns(storedSessionId: string, turns: AgentChatTurn[]) {
    homeDirectTurnsRef.current = turns;
    setHomeDirectTurns(turns);
    persistHomeDirectTurns(storedSessionId, turns);
  }
  ...
  async function startHomeTask(
    ...
    try {
      ...
      updateHandoff({ status: "failed", error: messageFromError(error) });
    }
  }
  ...
  async function submitHomeMessage(event?: FormEvent) {
    ...
    if (!message || importingFiles || textActionsDisabledReason) return;
    const profile = activeHermesProfile.name;
    const focusedModelTarget = captureFocusedSessionDefaultModelTarget();
    ...
    setDraft("");
    setCategory(null);
    rememberComposerDraft(submittedDraftKey, "", null, attachmentsRef.current);
    ...
    try {
      ...
      await enqueueHomeDirectChat(storedSessionId, async () => {
        ...
        const conversation = homeConversationContextFromTurns([...hermesTurns, ...contextTurns]);
      ...
      if (storedSessionId && userTurnId) {
        const failedUserTurnId = userTurnId;
      ...
      if (composerEditorRef.current?.isEmpty()) {
        composerEditorRef.current.setContent(message);
        draftRef.current = message;
        setDraft(message);
        rememberComposerDraft(submittedDraftKey, message, null, attachmentsRef.current);
      }
  ...
  async function submit(event?: FormEvent) {
    ...
    const message = draftRef.current.trim();
    if (homeMode && /^\/model(?:\s|$)/i.test(message)) {
      event?.preventDefault();
      setError("Model selection is available in focused sessions.");
      return;
    }
    if (
      homeMode &&
      ...
      return submitHomeMessage(event);
    }
  ...
  const homeConversationTurns = useMemo(
    ...
    [hermesTurns, homeDirectTurns, homeStreamingReply],
  );
  ...
  const homeGreetingVisible =
    homeMode &&
    (!homeConversationTurns.length ||
      homeConversationTurns.at(-1)!.createdAt.localeCompare(homeCheckIn.createdAt) < 0);
  ...
  const renderedTurnsSignature = chatTurnsSignature(
    homeMode ? homeConversationTurns : selectedHermesSessionId ? hermesTurns : taskTurns,
  );
  ...
  function startFocusedSessionFromHome() {
  ...
  const composer = renderAgentComposer({
    ...
    composerInSteerState: homeMode ? false : composerInSteerState,
    composerModelFlyout,
    composerModelOpen,
    composerModelPopoverRef,
    composerModelRootSearchRef,
  ...
  const detailContent = (
    <AgentDetailContent
      {...{
        ...
        home: homeMode
          ? {
              turns: homeConversationTurns,
              handoffs: homeTaskHandoffs,
              greeting: homeGreeting,
              greetingVisible: homeGreetingVisible,
              checkInCreatedAt: homeCheckIn.createdAt,
              nudgePrompts: homeNudgePrompts,
              pending: !homeStreamingReply && homeDirectPendingCount > 0,
              onPrefillNudge: prefillHomeNudge,
              onOpenTaskSession: onOpenHomeTaskSession,
              onRetryTask: retryHomeTask,
            }
          : undefined,
        listRef,
        newSessionMode,
