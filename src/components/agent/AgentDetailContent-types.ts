...
export type HomeDetailContent = {
  ...
  greeting: { salutation: string; question: string };
  greetingVisible: boolean;
  checkInCreatedAt: string;
  nudgePrompts: readonly string[];
  pending: boolean;
  onPrefillNudge: (prompt: string) => void;
  onOpenTaskSession?: (storedSessionId: string, title: string) => void;
  onRetryTask: (handoff: HomeTaskHandoff) => void;
};
