...
import { juneHomeDayKey, juneHomeDayLabel } from "../../lib/june-home";
...
export function AgentDetailContent(dependencies: RenderAgentDetailContentDependencies) {
  ...
  const onHermesApproval = useCallback<AgentChatTurnRowProps["onApproval"]>(
    ...
    [selectedHermesSessionId, stableRespondToApproval],
  );
  const onHermesClarify = useCallback<AgentChatTurnRowProps["onClarify"]>(
    ...
    [selectedHermesSessionId, stableRespondToClarify],
  );
  const onHermesSudo = useCallback<AgentChatTurnRowProps["onSudo"]>(
    ...
    [selectedHermesSessionId, stableRespondToSudo],
  );
  const onHermesSecret = useCallback<AgentChatTurnRowProps["onSecret"]>(
    ...
    [selectedHermesSessionId, stableRespondToSecret],
  );
  ...
  const homeHandoffsByTurnId = useMemo(() => {
    const handoffs = new Map<string, NonNullable<typeof home>["handoffs"][number]>();
    if (!home) return handoffs;
    for (const turn of home.turns) {
      const tool = turn.parts.find(
        (part) =>
          part.type === "tool" &&
          home.handoffs.some((handoff) => handoff.id === `home-task-${part.id}`),
      );
      const handoff =
        tool?.type === "tool"
          ? home.handoffs.find((candidate) => candidate.id === `home-task-${tool.id}`)
          : undefined;
    ...
    (turn) => {
      ...
      return (
        <AgentChatTurnRow
          ...
          onBranch={home ? undefined : onHermesBranch}
  ...
  const onTaskApproval = useCallback<AgentChatTurnRowProps["onApproval"]>(
    (part, choice) => {
      ...
      void stableRespondToApproval(
        ...
        part.id,
        choice,
        sessionUnrestricted(taskHermesSessionId),
      );
    },
    [stableRespondToApproval, taskHermesSessionId, taskId],
  );
  const onTaskClarify = useCallback<AgentChatTurnRowProps["onClarify"]>(
    ...
    [stableRespondToClarify, taskHermesSessionId, taskId],
  );
  const onTaskSudo = useCallback<AgentChatTurnRowProps["onSudo"]>(
    ...
    [stableRespondToSudo, taskHermesSessionId, taskId],
  );
  const onTaskSecret = useCallback<AgentChatTurnRowProps["onSecret"]>(
    ...
    [stableRespondToSecret, taskHermesSessionId, taskId],
  );
  ...
  return gallerySections ? (
    ...
    <div ref={listRef} className="agent-timeline" data-home="true">
      {home.turns.map((turn, index) => {
        const previous = index > 0 ? home.turns[index - 1] : undefined;
        ...
        return (
          <Fragment key={turn.id}>
            {dayMarker}
            {renderHermesTurn(turn)}
          </Fragment>
        );
      })}
      {home.greetingVisible ? (
        <>
          {home.turns.length > 0 &&
          juneHomeDayKey(home.checkInCreatedAt) !==
            juneHomeDayKey(home.turns.at(-1)?.createdAt ?? "") ? (
            <div className="agent-home-day">{juneHomeDayLabel(home.checkInCreatedAt)}</div>
          ) : null}
          <div className="agent-home-greeting">
            <span className="agent-home-greeting-mark" aria-hidden>
              <JuneBloom size={30} animated />
            </span>
            <h2>{home.greeting.salutation}</h2>
            <p>{home.greeting.question}</p>
          </div>
        </>
      ) : null}
      {home.turns.length === 0 ? (
        <section className="agent-home-nudges" aria-label="Suggestions">
          {home.nudgePrompts.map((prompt) => (
            <button key={prompt} type="button" onClick={() => home.onPrefillNudge(prompt)}>
              {prompt}
            </button>
          ))}
        </section>
      ...
      {browserApprovalCards}
      <AgentThinking variant="typing-bubble" visible={home.pending} />
    </div>
